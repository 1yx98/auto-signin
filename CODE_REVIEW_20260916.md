...
# 第十章：真实日志驱动的复审（2026-09-16 傍晚）

## 10.1 转折点：用户提供了 9 次真实运行日志

前九章全部建立在**静态推断**上。本章的数据来自 `logs/signin_20260916.log`
（1593 行、9 次运行），是**真实环境跑出来的**，价值高于此前所有推断。

**并因此推翻了我自己此前的两个结论：**

| 我此前说的 | 实际数据 | 结论 |
|---|---|---|
| 「已结束」识别率 40% | 今日 5 次触发 **5 次全中**（100%） | **我错了**，40% 是 Otsu 修复前的旧基线 |
| 只有 1 张真实样本 | 今日有 5 次真实触发记录 | **我错了**，样本比我说得多 |

> 谨慎界限：这 5 次统计的是"走到了 OCR 判据那一步"的样本，
> 不是全部 9 次运行（第 5、6 次压根没走到，是 `复核 0 次`）。
> 但足以说明：**只要几何+字迹双路命中，OCR 这一步基本不失手。**

## 10.2 用户澄清了问题的真实语义（最关键的一条）

**我原来的理解**：「已结束」= 今天已经签过了 → 判 fail 合理，只是识别率要提升。

**用户的实际语义**（2026-09-16 傍晚澄清）：

> 用户每天 **20:50** 才收到学校的晚点名推送。
> 在推送到来**之前**打开详情页，看到的是**昨天那条记录的「已结束」**。
> 「已结束」= **今天的还没推送，这是昨天的残留** —— 它是**正常状态，不是失败**。

**所以脚本把「已结束」判成 `fail` + 退出码 1 + 留失败现场 + 发飞书告警，
是彻底的误报。** 这不是识别率问题，是**语义判断错误**。

## 10.3 真根因：证据在导航层被丢弃（不是时间守卫失效）

这是本章最重要的技术发现，也是我一开始**猜错、靠日志纠正**的地方。

### 我最初的错误猜测
我以为时间守卫（`before_signin_start`/`_within_signin_window`）没生效。

### 日志给出的铁证
`logs/signin_20260916.log` 第 16:34:03，三行紧挨着：
...
**一句话概括：OCR 好不容易认出了最关键的那条证据，却在导航层被当成"匹配失败"扔掉了，
决策层只看到"导航失败"。**
...
3. `open_signin_entry` 传 `out=_out`，读 `_out.get("negative")` → `return "not_time"`
   （沿 L3399 既有链路 → 退出码 3、不告警、不留失败现场、不记自愈失败）

**关键：这条 `not_time` 链路本来就存在且正确，缺陷只是"事实到不了它那里"。**

## 10.4 三层回归断言（`test_ended_record_propagates_not_time`）

| 层 | 判据 | 方式 |
|---|---|---|
| ① | `out` 参数存在，且**真的被赋值**（不是签名占位） | AST 查 `ast.Assign` 节点 |
| ①.5 | 该赋值**必须在 OCR 否决分支内**（否则字迹不符也会被记成"已结束"） | AST 查行号包含关系 |
| ② | 导航层**真的传了** `out=`，且读了 `get("negative")` 并 `return "not_time"` | AST 查 `ast.Call.keywords` + `ast.If` |
| ③ | 不传 `out` 时行为与改动前**完全一致** | 运行时（AST 取真函数源码执行） |

**全部用 AST，不用字符串搜索** —— 修复代码的注释里就写着"应返回 not_time"这句话，
用字符串必然命中注释 → 断言恒真成摆设。本项目已因这类错误栽过三次。

## 10.5 负向测试（4 项，全部 REAL）

| # | 注入 | 结果 |
|---|---|---|
| ① | 删掉 `out` 参数 | **REAL**（失败 1 项） |
| ② | `out["negative"] = True` 改成 `False` | **REAL**（失败 1 项） |
| ③ | 导航层不传 `out=` | **REAL**（失败 1 项） |
| ④ | 导航层读了 `negative` 却 `return False` | **REAL**（失败 1 项） |

- 基线 sha256 `af386517...` → 4 轮注入+恢复后**校验仍一致**
- 每项单独验证，未抽查（遵循 MEMORY 4.7）
- ④ 第一次因锚点不唯一（`return "not_time"` 命中 9 处）被 SKIP，
  **改用多行唯一锚点后重跑**才拿到结果 —— 印证"SKIP 不等于通过"

## 10.6 验证结果

```
冒烟测试：通过 158 项，失败 0 项   （157 → 158，新断言确认已注册执行）
连跑 5 次：全部 158/158，结果一致
真实日志：1593 行不变，smoke_/[测试]/probe_ 命中 0
run 目录：17 点后未新建任何目录（SIGNIN_LOG_DIR_OVERRIDE 防污染生效）
负向测试：4/4 REAL，sha 校验通过
```

## 10.7 本章的方法论收获

### 收获 1：真实日志能推翻"很有道理的静态推断"，且推翻得不留情面
我在 10.1 表格里错了两次（40% 识别率、样本量），
在 10.3 里又错了一次（以为时间守卫失效）。
**三次都是同一个病：把"我读代码得到的印象"当成了"系统的实际行为"。**
→ **有真实日志时，先读日志，再读代码。读日志是为了发现"我没想到的路径"。**

### 收获 2：函数返回值的"表达力不足"是一种隐蔽的缺陷类型
`signed_detail_button` 返回 `None` 本身没错，错在**三种不同情况挤在同一个返回值里**。
这种缺陷**不会报错、不会告警、测试也不容易覆盖**，因为每个分支单独看都"正确"。
→ **排查信号：当一个函数返回 None/False 后，调用方只能靠"猜"来分支时，
   就该问"是不是丢掉了信息"。**

### 收获 3：区分"正常状态"与"故障"是产品的核心职责，不是细节
用户真正的痛点是：**脚本把"今天还没推送"报成了"签到失败"**。
这导致每天的失败现场、飞书告警全是噪音 —— 而噪音会训练人忽略告警，
**最终连真故障一起忽略**（与 MEMORY 第 3 条"偶发失败比稳定失败更危险"同源）。
→ **"误报正常状态"和"漏报真故障"是同一枚硬币，都会摧毁告警的可信度。**

## 10.8 遗留与下一步

**本次未修（需用户决策）：**
1. **「微信窗口激活后仍不可见」** —— 今日 9 次运行中**至少 6 次**出现，
   第 1、2、3 次直接死在这里。**这是比 OCR 更靠前的失败源，收益最大。**
2. **「进入微信」盲点越界频繁触发** `conf=0.269~0.391`
   —— 远低于 0.80 阈值却仍被当候选，说明模板匹配在乱命中。
3. **第 6 次运行 OCR 复核 0 次** —— 两轮跑完 5 个步骤却一次没进判据，
   疑似"导航到详情页"链路某处静默退出。
4. **几何+字迹判据在「已结束」上会误判成「已签到」**
   —— 今日 5 次全部靠 OCR 兜住。**若某次 OCR 读不出（veto-only 会放行），
   就会变成假成功。今天没出事 ≠ 防线稳，是运气。** 这条最该深挖。

**仍然成立的老结论：**
- 端到端真机签到从未跑过（沙箱无 GUI 登录态），GUI 结论仍是静态推断
- `logs/samples/` 里「已签到」「已结束」仍各只 1 张（不过今日日志提供了新素材）
