...
## 10.9 真实验证：修复在真实环境确认生效（17:56 手跑）

用户手动运行一次（时间 17:56，时间窗 20:50 之外，页面停在昨天已结束记录），
**正好命中目标场景**：

```
17:58:38 [已签到判据] 几何 + 字迹双路一致 → 字迹符合「已签到」
17:58:38 [OCR] 详情页按钮文字=「已结束」→ 否决
17:58:38 [WARNING] [导航] 第4步...未找到'进入按钮'，且 OCR 读到否定词（历史/未开放记录）
                  ——判定为 not_time（不是失败；通常是今日签到尚未推送）   ← 新增的这行
17:58:38 [流程] 签到未开始（不在时段），返回 not_time
17:58:39 最终结果=not_time  退出码=3  含义：不在签到时段/任务已结束（并非失败）
17:58:40 [自愈] 非签到时段，不更新故障状态
17:58:40 [台账] 已记录：2026-09-16 17:58:40 → not_time
```
...
| | 修复前（16:34） | 修复后（17:58） |
|---|---|---|
| 导航层 | `滚动后仍未找到，终止` → `False` | `OCR 读到否定词 → 判定为 not_time` |
| 最终结果 | **fail** / 退出码 **1** | **not_time** / 退出码 **3** |
| 自愈状态 | `记录失败 S3/NAV_ENTRY_FAIL（连续1次）` | `非签到时段，不更新故障状态` |
| 失败现场 | 有 FAIL_*.png | **无 FAIL_*.png（已核实）** |
| 台账 | `→ fail` | `→ not_time` |

**退出码 3 = "并非失败"，正是用户要求的语义。修复确认生效。**

## 10.10 新增真实样本：「已结束」样本量翻倍（1 → 2）

从本次成功运行的现场截图中提取到一张**高质量真实样本**：

`logs/samples/20260916_1758_ended_detail_已结束.jpg`

它的价值超出普通样本 —— **它把整个缺陷现场完整记录下来**：
- 页面显示 `每日签到 2026.09.15 20:50 至 2026.09.15 22:00`（**昨天的**）
- 签到状态：**已结束**；签到时间：`2026-09-15 21:16`
- 底部灰色按钮：**「已结束」**
- **并且"几何 + 字迹"两路同时判成「已签到」** —— 两种判据的冲突在这张图上一目了然

**这个信息很有价值**：详情页上**日期是比按钮文字更直接的证据**
（9/16 打开却显示 9/15）。未来若要彻底根治，可考虑读日期而不只读按钮文字。

**验证**（确定性定位器 + 真实 OCR 管线）：
```
_find_gray_bar → 1 个灰条 {'kind':'gray','cx':1442,'cy':1205,'w':761,'h':90}
               （与运行日志 gray@(1442,1206)764x90 吻合）
ocr_button_text → '已结束'（完整词，非残缺）
ocr_veto_signed → False（否决生效）
```

**已被现有回归自动纳入**（`test_ocr_real_samples` 按文件名关键字 glob）：
```
[PASS] OCR 真实样本：「已结束」能读出  -- 「已结束」在 2/2 张真实样本上读出
```

→ MEMORY 里列为"最该优先加固"的样本量问题，**由此翻倍**。
（样本目录已 gitignore，含个人界面信息，不进仓库。）

## 10.11 我自己造成的一次操作污染（诚实记录）

用户发现 D 盘多出 `D:/_tmp_probe_logs/`。**是我造的，不是签到系统产生的。**

原因：17:47 跑运行时验证脚本时，用
`os.path.join(os.path.dirname(os.path.abspath(".")), "_tmp_probe_logs")`
算临时目录 —— `abspath(".")` 在当时环境解析成 `D:/`（不是项目根），
于是日志重定向目录落到 **D 盘根目录**。内容 11 行，全带 `(模拟)` 字样，纯测试输出。

**值得注意**：真正的防污染机制**生效了** ——
真实 `logs/signin_20260916.log` 从 1593 涨到 1744 行，**全部是真实运行记录，零测试噪音**。
副作用只是"临时目录位置算错"。

**教训（已写进 MEMORY）**：
写验证/诊断脚本算临时目录，**一律用 `tempfile.mkdtemp()`**，
**绝不用 `os.path.abspath(".")` / `os.getcwd()`** ——
后者的结果取决于"脚本从哪个工作目录被启动"，与"脚本放在哪"无关。
（项目自身干净：`smoke_test.py` 全部用 `tempfile`，无一处用 `abspath(".")`。）

清理：内容已确认是垃圾；但回收站 API（`Add-Type` 编译 .NET、
`Shell.Application` COM）**均被沙箱安全策略拦截**，`gio`/`trash-put` 也不存在
→ **不绕过安全策略强删**，改由用户手动删除。
