...
def main():
    ...
    if len(sys.argv) > 1:
        try:
            ...
            return 2
        # 【2026-09-16 修复·P2-5】加范围校验。
        # 原来只校验了"能不能转成 int"，于是：
        #   · 负数（如 `report.py -10`）→ recent_records 的日期下界跑到未来
        #     → 一条都查不到，只打印"最近 -10 天没有记录"，看着像"系统坏了"；
        #   · 0 → 同样查不到，误导性更强（"最近 0 天"这个说法本身就不成立）；
        #   · 超大值（`report.py 99999999`）→ 全表扫描 + 逐条解析，
        #     台账长大后每次都要等几秒，而且没有任何提示。
        # 这里给出明确报错并返回 2（与"参数写错"的既有约定一致），
        # 而不是让它拿着不合法的参数继续往下跑。
        # 上限 3650（约 10 年）是"远超任何合理使用场景、但能挡住 9 位数"的量级。
        if not (1 <= days <= 3650):
            print("天数必须在 1~3650 之间（你给的是 %d）。用法：python report.py [天数]" % days)
            return 2
